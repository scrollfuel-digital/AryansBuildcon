    ...
    >
      ...
      {open && (
        <div className="px-5 pb-5 bg-white border-t border-black/5">
          <p className="font-sans text-sm text-grey font-light leading-relaxed pt-3">
            {a}
          </p>
        </div>
      )}
    ...
    <div className="space-y-24">
      ...
      >
        <div className="text-center space-y-3">
          <span className="font-sans text-[10px] font-bold uppercase tracking-[0.25em] text-accent-rust block">
            Simple & Transparent
          </span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {buyingSteps.map((s, idx) => (
            <motion.div
              ...
              className="relative bg-white rounded-2xl p-5 border border-black/10 shadow-md text-center space-y-3 group hover:border-accent-gold hover:-translate-y-1 transition-all duration-300"
            >
              <div className="w-10 h-10 bg-charcoal text-accent-gold rounded-xl flex items-center justify-center font-serif text-sm font-bold mx-auto group-hover:bg-accent-gold group-hover:text-white transition-colors duration-300">
                {s.step}
              </div>
              ...
              {idx < buyingSteps.length - 1 && (
                <ArrowRight className="w-3.5 h-3.5 text-accent-gold/40 absolute -right-2 top-1/2 -translate-y-1/2 hidden lg:block" />
              )}
            </motion.div>
          ))}
        </div>
        ...
        <div className="relative z-10 space-y-6 max-w-2xl mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 pt-2">
            <a
              ...
              className="flex items-center gap-2 bg-accent-gold hover:bg-accent-dark-gold text-white px-6 py-3 rounded-full font-sans text-xs font-medium uppercase tracking-[0.1em] transition-all duration-300 hover:-translate-y-0.5 shadow-lg"
            >
              <Phone className="w-4 h-4" /> Call Today
            </a>
            <button
              onClick={() => navigate("/contact")}
              className="flex items-center gap-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white px-6 py-3 rounded-full font-sans text-xs font-medium uppercase tracking-[0.1em] transition-all duration-300 hover:-translate-y-0.5 cursor-pointer"
            >
